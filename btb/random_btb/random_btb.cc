...
BASIC_BTB_ENTRY *basic_btb_get_random_entry(uint8_t cpu, uint64_t set) {
    return &(basic_btb[cpu][set][std::rand() % BASIC_BTB_WAYS]);
}
...
void O3_CPU::initialize_btb() {
    std::cout << "Random BTB sets: " << BASIC_BTB_SETS
              << " ways: " << BASIC_BTB_WAYS
    ...
    if (branch_type == BRANCH_RETURN) {
        ...
        if (btb_entry == nullptr) {
            ...
            if ((branch_target != 0) && taken) {
                ...
                uint64_t set = basic_btb_set_index(ip);
                auto repl_entry = basic_btb_get_random_entry(cpu, set);
