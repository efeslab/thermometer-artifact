...
//    uint64_t lru = 0;
    ...
    BASIC_BTB_ENTRY(uint64_t ip, uint64_t target) :
        ip_tag(ip), target(target) {}
};

class Unlimited {
    vector<unordered_map<uint64_t, BASIC_BTB_ENTRY>> btb;
...
public:
    ...
    BASIC_BTB_ENTRY *find_entry(uint8_t cpu, uint64_t ip) {
        auto it = btb[cpu].find(ip);
        if (it != btb[cpu].end())
            return &(it->second);
        return nullptr;
    }

    void insert_to_btb(uint8_t cpu, uint64_t ip, uint64_t target) {
        btb[cpu].emplace(ip, BASIC_BTB_ENTRY(ip, target));
    }
};

//vector<vector<vector<BASIC_BTB_ENTRY>>> basic_btb;
Unlimited unlimited_btb;
...
std::pair<uint64_t, uint8_t> O3_CPU::btb_prediction(uint64_t ip, uint8_t branch_type, uint64_t *latency) {
    ...
    if (branch_type == BRANCH_RETURN) {
        ...
        auto btb_entry = unlimited_btb.find_entry(cpu, ip);
    ...
    if (branch_type == BRANCH_RETURN) {
        ...
        if (btb_entry == NULL) {
            if ((branch_target != 0) && taken) {
                ...
                unlimited_btb.insert_to_btb(cpu, ip, branch_target);
//                uint64_t set = basic_btb_set_index(ip);
...
void O3_CPU::prefetch_btb(uint64_t ip, uint64_t branch_target, uint8_t branch_type, bool taken, bool to_stream_buffer) {
    // No need to add a stream buffer since the size is unlimited.
    if (branch_type != BRANCH_RETURN &&
