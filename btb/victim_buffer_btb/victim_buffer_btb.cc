...
// Note: we only use ip_tag and target in the victim_btb, which is 128 bits in total.
// Size of victim buffer is 128-entry.

class VictimBuffer {
    deque<BASIC_BTB_ENTRY> victim_buffer;
    uint64_t victim_buffer_size = 128;

public:
    VictimBuffer() = default;

    BASIC_BTB_ENTRY *find_entry(uint64_t ip) {
        for (auto &a : victim_buffer) {
            if (a.ip_tag == ip) {
                return &a;
            }
        }
    ...
    void add_to_victim_buffer(BASIC_BTB_ENTRY *repl_entry) {
        if (victim_buffer.size() >= victim_buffer_size)
            victim_buffer.pop_front();
        assert(victim_buffer.size() < victim_buffer_size);
        victim_buffer.emplace_back(repl_entry->ip_tag, repl_entry->target);
    }
};
    ...
    if (branch_type == BRANCH_RETURN) {
        ...
        if (btb_entry == nullptr) {
            ...
            if ((branch_target != 0) && taken) {
                ...
                // repl_entry is evicted, so add it to the victim buffer
                victim_buffer.add_to_victim_buffer(repl_entry);
