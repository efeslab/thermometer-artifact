...
public:
    ...
    // TLBs and caches
    CACHE ITLB{"ITLB", ITLB_SET, ITLB_WAY, ITLB_WQ_SIZE, ITLB_RQ_SIZE, ITLB_PQ_SIZE, ITLB_MSHR_SIZE, ITLB_HIT_LATENCY,
               ITLB_FILL_LATENCY, ITLB_MAX_READ, ITLB_MAX_WRITE},
            DTLB{"DTLB", DTLB_SET, DTLB_WAY, DTLB_WQ_SIZE, DTLB_RQ_SIZE, DTLB_PQ_SIZE, DTLB_MSHR_SIZE, DTLB_HIT_LATENCY,
                 DTLB_FILL_LATENCY, DTLB_MAX_READ, DTLB_MAX_WRITE},
            STLB{"STLB", STLB_SET, STLB_WAY, STLB_WQ_SIZE, STLB_RQ_SIZE, STLB_PQ_SIZE, STLB_MSHR_SIZE, STLB_HIT_LATENCY,
                 STLB_FILL_LATENCY, STLB_MAX_READ, STLB_MAX_WRITE},
            L1I{"L1I", L1I_SET, L1I_WAY, L1I_WQ_SIZE, L1I_RQ_SIZE, L1I_PQ_SIZE, L1I_MSHR_SIZE, L1I_HIT_LATENCY,
                L1I_FILL_LATENCY, L1I_MAX_READ, L1I_MAX_WRITE},
            L1D{"L1D", L1D_SET, L1D_WAY, L1D_WQ_SIZE, L1D_RQ_SIZE, L1D_PQ_SIZE, L1D_MSHR_SIZE, L1D_HIT_LATENCY,
                L1D_FILL_LATENCY, L1D_MAX_READ, L1D_MAX_WRITE},
            L2C{"L2C", L2C_SET, L2C_WAY, L2C_WQ_SIZE, L2C_RQ_SIZE, L2C_PQ_SIZE, L2C_MSHR_SIZE, L2C_HIT_LATENCY,
                L2C_FILL_LATENCY, L2C_MAX_READ, L2C_MAX_WRITE};
    ...
    // constructor
    O3_CPU(uint32_t cpu, uint64_t warmup_instructions, uint64_t simulation_instructions) : cpu(cpu), begin_sim_cycle(
            warmup_instructions), warmup_instructions(warmup_instructions), simulation_instructions(
...
//	initialize_btb();
    ...
    // btb
    std::pair<uint64_t, uint8_t> btb_prediction(uint64_t ip, uint8_t branch_type, uint64_t *latency = nullptr);

    void initialize_btb();
    void update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type);
    void prefetch_btb(uint64_t ip, uint64_t branch_target, uint8_t branch_type, bool taken, bool to_stream_buffer = false);
    ...
    static std::string find_trace_short_name(std::string &full_path, NameKind name_kind);

    void open_btb_record(const char *mode, bool is_shotgun) {
        string directory_name = "/mnt/storage/shixins/champsim_pt/";
        auto filename = find_trace_short_name(trace_name, O3_CPU::NameKind::TRACE) + ".txt";
        if (is_shotgun) {
            string cond_dir_name = directory_name + "btb_conditional_record/" + filename;
            btb_conditional_record = fopen(cond_dir_name.c_str(), mode);
            string uncond_dir_name = directory_name + "btb_unconditional_record/" + filename;
            btb_unconditional_record = fopen(uncond_dir_name.c_str(), mode);
            assert(btb_conditional_record != nullptr && btb_unconditional_record != nullptr);
        } else {
            if (IFETCH_BUFFER_SIZE != 192) {
                filename = ("fdip" + std::to_string(IFETCH_BUFFER_SIZE)) + "_" + filename;
            }
    ...
    void l1i_prefetcher_branch_operate(uint64_t ip, uint8_t branch_type, uint64_t predicted_branch_target,
    ...
    void l1i_prefetcher_resolved_branch_operate(ooo_model_instr &instr, bool decode_stage);
    ...
    unordered_map<uint64_t, vector<pair<uint64_t, uint64_t>>> *twig_prefetch_match = nullptr;
};
